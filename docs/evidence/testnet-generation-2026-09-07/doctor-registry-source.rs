...
impl DoctorRegistry {
    ...
    /// Bind the governing account atomically when this instance is deployed.
    pub fn __constructor(env: Env, admin: Address) {
        env.storage().instance().set(&DataKey::Admin, &admin);
    }

    /// Legacy entry point retained for callers; constructed instances reject it.
    ...
    pub fn grant_permission(
        ...
        Self::require_admin(&env)?;
        let doctor = Self::get_doctor(env.clone(), doctor_wallet.clone())?;
    ...
    /// Returns false for unknown/revoked doctors or ungranted permissions.
    /// Historical permissions remain available through get_permissions.
    pub fn has_permission(env: Env, doctor_wallet: Address, permission: Symbol) -> bool {
        if !Self::is_authorized(env.clone(), doctor_wallet.clone()) {
            return false;
        }
        let key = DataKey::Permissions(doctor_wallet);
        let perms: Vec<Symbol> = env
