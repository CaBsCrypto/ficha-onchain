...
pub trait DispensaryRegistryTrait {
...
impl PrescriptionSoulbound {
    ...
    pub fn activate(env: Env, caller: Address, prescription_id: u64) -> Result<(), Error> {
        ...
        Self::store_prescription(&env, &rx);
        env.events().publish((symbol_short!("rx_act"), caller), prescription_id);
    ...
    /// La autorización se consulta en el registro configurado y falla cerrada.
    ///
    /// Transiciones:
    ...
    pub fn dispense(
        ...
        // A missing contract, placeholder account, malformed response or registry
        // error never grants dispensing rights.
        if !matches!(DispensaryRegistryClient::new(&env, &registry)
            .try_is_authorized(&dispensary), Ok(Ok(true))) {
            return Err(Error::Unauthorized);
        }
        if units == 0 {
            return Err(Error::InvalidQuantity);
    ...
    pub fn block(env: Env, prescription_id: u64) -> Result<(), Error> {
        ...
        Self::store_prescription(&env, &rx);
        env.events().publish((symbol_short!("rx_block"), admin), prescription_id);
    ...
    pub fn is_valid(env: Env, id: u64) -> bool {
        match Self::load_prescription(&env, id) {
            Ok(rx) => {
                matches!(rx.status, Status::Active | Status::PartiallyDispensed)
                    && rx.balance > 0 && env.ledger().timestamp() < rx.expires_at
            }
            Err(_) => false,
        }
    }
