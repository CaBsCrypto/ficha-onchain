...
fn test_register_doctor_success() {
    ...
    let client = DoctorRegistryClient::new(&env, &env.register(DoctorRegistry, (admin.clone(),)));
...
#[test]
fn test_permission_grants_require_an_authorized_doctor() {
    let env = Env::default();
    env.mock_all_auths();
    let admin = Address::generate(&env);
    let client = DoctorRegistryClient::new(&env, &env.register(DoctorRegistry, (admin,)));
    let doctor = Address::generate(&env);
    assert_eq!(client.try_grant_permission(&doctor, &PERM_CANNABIS), Err(Ok(Error::DoctorNotFound)));
    client.register_doctor(&doctor, &dr_name(&env), &dr_license(&env));
    client.revoke_doctor(&doctor);
    assert_eq!(client.try_grant_permission(&doctor, &PERM_CANNABIS), Err(Ok(Error::NotAuthorized)));
...
#[test]
fn test_revocation_disables_permissions_but_keeps_history() {
    let env = Env::default();
    env.mock_all_auths();
    let admin = Address::generate(&env);
    let client = DoctorRegistryClient::new(&env, &env.register(DoctorRegistry, (admin.clone(),)));
