...
// Minimal registry fixture implementing the real is_authorized ABI. Only the
// explicitly configured wallet is authorized; all other addresses fail closed.
#[soroban_sdk::contract]
struct TestDispensaryRegistry;
#[soroban_sdk::contractimpl]
impl TestDispensaryRegistry {
    pub fn __constructor(env: Env, authorized: Address) {
        env.storage().instance().set(&0u32, &authorized);
    }
    pub fn is_authorized(env: Env, dispensary: Address) -> bool {
        env.storage().instance().get::<_, Address>(&0u32) == Some(dispensary)
    }
}
...
fn setup(
    ...
    let registry_id = env.register(DoctorRegistry, (admin.clone(),));
    let registry = DoctorRegistryClient::new(env, &registry_id);

    let dispensary = Address::generate(env);
    let dispensary_registry = env.register(TestDispensaryRegistry, (dispensary.clone(),));
