  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (alive && !posting.current && epoch === requestEpoch.current) { setReview(body); setError(""); }
      } catch (err) {
  ...
  async function submit(action: Action) {
    ...
    try {
      const response = await authedFetch("/api/admin/doctor-authorizations", {
        ...
        body: JSON.stringify({ doctorId: doctor.id, action, confirmed: true, synthetic: true }),
      });
