...
export function PrivateConsultations({ role }: { role: PortalRole }) {
  ...
  return <section className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-xl font-semibold text-slate-900">Mis consultas</h1><p className="mt-1 text-sm text-slate-500">Reserva, asistencia y permiso de emisión son pasos separados.</p></div>{role === 'patient' && <button data-patient-primary onClick={() => setBooking(true)} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white">Reservar consulta</button>}</div>
